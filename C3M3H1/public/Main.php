...
{
    $monster = fake()->numberBetween(1, 2);
    $obstacle = fake()->numberBetween(1, 5);
    $treasure = fake()->numberBetween(1, 10);

    $controllableOption = generateMapOptions(false, true);
    $monsterOption = generateMapOptions(false, true, true, $monster, 0.2);
    $obstacleOption = generateMapOptions();
    $treasureOption = generateMapOptions(generable: true, number: $treasure, rate: 0.5);
